...
class PembelianBahan extends Model
{
    ...
    protected static function booted()
    {
        static::created(function ($pembelian) {
            ...
            if ($pembelian->bahanBaku) {
                $pembelian->bahanBaku->increment('stok', $pembelian->jumlah);
            }
        });
        ...
        static::deleted(function ($pembelian) {
            if ($pembelian->bahanBaku) {
                $pembelian->bahanBaku->decrement('stok', $pembelian->jumlah);
            }
        });
    }
    ...
    public static function generateKode()
    {
        ...
        if (!$last) return 'PB001';

        $num = (int) substr($last->kode_pembelian, 2);
        return 'PB' . str_pad($num + 1, 3, '0', STR_PAD_LEFT);
    }
}